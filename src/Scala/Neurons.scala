...
class neuron_top(coreID : Int) extends BlackBox(Map("io_coreID" -> coreID)) {// with HasBlackBoxResource {
  val io = IO(new Bundle() {
    //Clock and Reset Ports must be explicitly declared
    //val io_coreID   = Input(UInt()) //test
    ...
    // val io_spikes   = Output(Vec(EVALUNITS, Bool()))
